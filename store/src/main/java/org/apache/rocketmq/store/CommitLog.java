...
//                            tagsCode = storeTimestamp + ticksDuration * Long.valueOf(propertiesMap.get(MessageConst.PROPERTY_DURATION));
                            tagsCode = System.nanoTime() + ticksDuration * Long.valueOf(propertiesMap.get(MessageConst.PROPERTY_DURATION));
    ...
    public CompletableFuture<PutMessageResult> asyncPutMessage(final MessageExtBrokerInner msg) {
        ...
        //开启warmMappedFile并且创建的mapFile空间不足以写入当前消息时，mlock解锁，释放mmap空间
        ...
        //submitFlushRequest会同步/异步刷盘，flush方法底层会调用对应的堆外内存或者直接申请内存
        //堆外内存好处就是内存池。。。
    ...
    public void handleDiskFlush(AppendMessageResult result, PutMessageResult putMessageResult, MessageExt messageExt) {
        ...
        if (FlushDiskType.SYNC_FLUSH == this.defaultMessageStore.getMessageStoreConfig().getFlushDiskType()) {
            ...
            if (messageExt.isWaitStoreMsgOK()) {
                ...
                //唤醒处理刷盘请求写磁盘线程，处理刷盘请求线程和提交刷盘请求之前的协调，
                // 通过CountDownLatch（1）操作，通过控制hasNotified状态来实现写队列和读队列的交换
                service.wakeup();
            }
        }
        //异步刷盘
    ...
    public SelectMappedBufferResult getMessage(final long offset, final int size) {
        ...
        if (mappedFile != null) {
            int pos = (int) (offset % mappedFileSize);
            //找到offset+size的selectMappedbuffer
...
//                    2.写入映射文件字节缓冲区(mappedByteBuffer) ----> 映射文件字节缓冲区(mappedByteBuffer)flush
    ...
    class GroupCommitService extends FlushCommitLogService {
        // 两个队列，读写请求分离
        // 刷盘服务写入请求队列
        //这里两个list原因就是，当线程处理队列的时候，会调用sync锁住线程，因此锁住的线程不能使用，创建一个read、一个write，本意就是不阻塞写请求
                ...
                if (!this.requestsRead.isEmpty()) {
                    ...
                    for (GroupCommitRequest req : this.requestsRead) {
                        ...
                        for (int i = 0; i < 2 && !flushOK; i++) {
                            //0代码立刻刷盘，不管缓存中消息有多少：调用mappedFile的flush方法， 由于数据会在fileChannel或者MappedByteBuffer中，则调用对应的force
    ...
    class DefaultAppendMessageCallback implements AppendMessageCallback {
        ...
        public AppendMessageResult doAppend(final long fileFromOffset, final ByteBuffer byteBuffer, final int maxBlank,
            ...
            if ((msgLen + END_FILE_MIN_BLANK_LENGTH) > maxBlank) {
                ...
                //返回的end_of_file会调用mapfile的munlock，解锁当前mmap一直在内存的映射
